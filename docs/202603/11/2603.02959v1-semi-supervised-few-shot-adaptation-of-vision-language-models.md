...
## 摘要
在大型异构数据源上预训练的视觉-语言模型（VLMs）正变得越来越流行，它们提供了丰富的多模态嵌入，能够高效地迁移到新任务中。一个特别相关的应用是少样本自适应，即仅利用极少数标注示例，通过多模态线性探测来调整模型。在医学影像领域，专门的 VLMs 在零样本和少样本图像分类中展现出极具前景的性能，这对于缓解专家标注的高昂成本具有重要价值。然而，在极低样本的情况下仍面临挑战：医学任务中固有的类别不平衡通常会导致类别代表性不足，从而损害模型的整体性能。为了解决这一局限性，我们提出利用无标注数据，引入一种高效的半监督求解器，在少样本自适应过程中传播基于文本信息的伪标签。所提方法为 VLMs 的自适应实现了更低预算的标注流程，在低样本情况下可减少 50% 以上的标注工作量。

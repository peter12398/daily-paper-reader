...
title_zh: AG-VAS：基于大语言多模态模型的锚点引导零样本视觉异常分割
...
method: 提出AG-VAS框架，引入三个可学习语义锚点令牌，并结合语义-像素对齐模块（SPAM）与锚点引导掩码解码器（AGMD）进行精确分割。
result: 在六个工业和医疗异常检测基准测试中，AG-VAS在零样本设置下均取得了持续领先的最先进性能。
conclusion: 该研究通过锚点引导的语义建模和跨模态对齐，有效提升了大模型在零样本视觉异常分割任务中的精确度和泛化能力。
---

## 摘要
大语言多模态模型 (LMMs) 展现出强大的任务泛化能力，为零样本视觉异常分割 (ZSAS) 提供了新的机遇。然而，现有的基于 LMM 的分割方法仍面临根本性的局限：异常概念本质上是抽象且依赖上下文的，缺乏稳定的视觉原型，且高层语义嵌入与像素级空间特征之间的弱对齐阻碍了精确的异常定位。为了应对这些挑战，我们提出了 AG-VAS (Anchor-Guided Visual Anomaly Segmentation)，这是一个通过三个可学习的语义锚点标记——[SEG]、[NOR] 和 [ANO] 扩展 LMM 词汇表的新框架，建立了一个统一的锚点引导分割范式。具体而言，[SEG] 作为一个绝对语义锚点，将抽象的异常语义转化为显式的、具有空间基础的视觉实体（如孔洞或划痕），而 [NOR] 和 [ANO] 则作为相对锚点，建模跨类别的正常与异常模式之间的上下文对比。为了进一步增强跨模态对齐，我们引入了一个语义-像素对齐模块 (SPAM)，用于将语言级语义嵌入与高分辨率视觉特征相对齐，以及一个锚点引导掩码解码器 (AGMD)，通过锚点条件的掩码预测实现精确的异常定位。此外，我们构建了 Anomaly-Instruct20K，这是一个大规模指令数据集，将异常知识组织成外观、形状和空间属性的结构化描述，促进了所提语义锚点的有效学习与整合。在六个工业和医疗基准测试上的广泛实验表明，AG-VAS 在零样本设置下一致地达到了最先进的性能。

...
## 摘要
医疗基础模型展现出从大规模、多样化数据集中学习广泛泛化特征的潜力。这可以作为实现可靠的跨模态泛化以及仅需少量任务特定样本即可快速适应新的任务特定目标的基础。然而，由于缺乏公开、标准化且可重复的评估框架，相关证据仍然有限，因为现有的公开基准通常分散在特定任务、器官或模态的设置中，限制了对跨任务泛化能力的评估。我们推出了 UNICORN，这是一个旨在统一协议下系统评估医疗基础模型的公开基准。为了隔离表示质量，我们基于一种新颖的两步框架构建了该基准，该框架通过标准化的少样本自适应（few-shot adaptation）将模型推理与任务特定评估解耦。作为核心设计选择，我们构建了源自临床相关队列的间接访问隔离测试集，并提供标准化的评估代码和开放基准平台上的提交接口。性能被聚合为单一的 UNICORN 分数，这是我们引入的一种新指标，旨在支持跨不同医学领域、模态和任务类型的基础模型直接比较。UNICORN 测试数据集包含来自 8 个国家 17 家机构的 2,400 多名患者的数据，其中包括 3,700 多个视觉案例和 2,400 多份临床报告。该基准涵盖了 8 个解剖区域和 4 种成像模态。任务特定排行榜和聚合排行榜均可实现易于访问、标准化且可重复的评估。通过标准化多任务、多模态评估，UNICORN 为医疗基础模型的可重复基准测试奠定了基础。数据、基准方法和评估平台可通过 unicorn.grand-challenge.org 公开获取。
